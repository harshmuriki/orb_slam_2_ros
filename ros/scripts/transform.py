...
if __name__ == '__main__':
    ...
    listener.waitForTransform("/map", "/base", rospy.Time(), rospy.Duration(4.0))
    ...
    while not rospy.is_shutdown():
        try:
            (transaux2cam,rotaux2cam) = listener.lookupTransform('/map', '/base', rospy.Time(0))
        ...
        correct_trans = [0,0,0]
        correct_rot = quaternion_from_euler(1.5708,0,1.5708,'rxyz')
        correct_world_rot = quaternion_from_euler(0,1.5708,0,'rxyz')
        final_rot = quaternion_from_euler(0,1.5708,-1.5708,'rxyz')
        correct_trans_mat = tf.transformations.translation_matrix(correct_trans)
        correct_rot_mat = tf.transformations.quaternion_matrix(final_rot)
        ...
        br.sendTransform(trans,
                         ...
                         "test_1",
                         "world")
        br.sendTransform(correct_trans,
                        correct_rot,
                        rospy.Time.now(),
                        "test_2",
                        "test_1")
        br.sendTransform(transaux2cam,
                        tf.transformations.quaternion_from_matrix(final_new),
                        rospy.Time.now(),
